...
- (void)resizeViewsIfWidthChanged
{
    ...
    if(size.width != _lastSize.width){
        ...
        _lastSize = size;
        CGFloat height = MIN(self.manager.settings.menuMonthHeight, size.height);
        ...
        _leftView.frame = CGRectMake(_leftView.frame.origin.x, 0, _scrollView.frame.size.width, height);
        _centerView.frame = CGRectMake(_centerView.frame.origin.x, 0, _scrollView.frame.size.width, height);
        _rightView.frame = CGRectMake(_rightView.frame.origin.x, 0, _scrollView.frame.size.width, height);
    }
}
...
- (void)repositionViews
{
    ...
    switch (_pageMode) {
        case JTCalendarPageModeFull:
            _scrollView.contentSize = CGSizeMake(size.width * 3, height);
            
            _leftView.frame = CGRectMake(0, 0, size.width, size.height);
            _centerView.frame = CGRectMake(size.width, 0, size.width, height);
            _rightView.frame = CGRectMake(size.width * 2, 0, size.width, height);
        ...
        case JTCalendarPageModeCenter:
            ...
            _leftView.frame = CGRectMake(- size.width, 0, size.width, height);
            _centerView.frame = CGRectMake(0, 0, size.width, height);
            _rightView.frame = CGRectMake(size.width, 0, size.width, height);
            
            _scrollView.contentOffset = CGPointZero;
            break;
        case JTCalendarPageModeCenterLeft:
            _scrollView.contentSize = CGSizeMake(size.width * 2, height);
            
            _leftView.frame = CGRectMake(0, 0, size.width, height);
            _centerView.frame = CGRectMake(size.width, 0, size.width, height);
            _rightView.frame = CGRectMake(size.width * 2, 0, size.width, height);
